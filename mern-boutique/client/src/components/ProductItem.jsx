...
const ProductItem = ({ id, image, name, price, originalPrice, isNew = false, rating, category, subCategory, showRemoveButton = false }) => {
  ...
  // Debug logging for the originalPrice issue
  useEffect(() => {
    // Only log for products with specific conditions to avoid flooding the console
    // Only log for the first few products to avoid console spam
    if (id && id.endsWith('3')) {
      console.log(`ProductItem DEBUG - ${name} (${id}):`, {
        price: price,
        priceType: typeof price,
        originalPrice: originalPrice,
        originalPriceType: typeof originalPrice,
        hasOriginalPrice: originalPrice !== undefined && originalPrice !== null, 
        priceToString: `${price}`,
        originalPriceToString: `${originalPrice}`,
        currencySymbol: currency
      });
    }
  }, [id, name, price, originalPrice, currency]);
  ...
  return (
    ...
    >
      ...
      {/* Discount Badge - Repositioned to bottom right */}
      ...
      {/* Wishlist Button - Positioned outside the product click area */}
      ...
      <div
        className="relative overflow-hidden cursor-pointer aspect-[3/4] bg-gray-100"
      ...
      >
        ...
        {/* Primary Image */}
        <img
          className={`w-full h-full object-cover transition-transform duration-1000 ease-out z-10 relative ${isHovered ? 'scale-110' : 'scale-100'} ${imageLoaded ? 'opacity-100' : 'opacity-0'}`}
        ...
        {secondaryImage && (
          <img
            className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-700 ease-in-out z-10 ${isHovered ? 'opacity-100' : 'opacity-0'}`}
            src={secondaryImage}
        ...
        {/* Overlay with Quick Actions - only the overlay is hidden until hover, not the image */}
        <div 
